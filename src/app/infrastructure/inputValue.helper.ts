...
export class InputValue {
  ...
  static removeCharactersHoldSpace(obj: string): string {
    if (!obj) {
      return "";
    }
    let str;
  ...
  static findDuplicates(arr: any[]): any[] {
    return arr.filter((item, index) => arr.indexOf(item) !== index);
  }

  static getCountDuplicates(arr: any[], element: any): number {
    const result: any[] = arr.filter(item => item === element);
    const countDuplicate: number = result.length;
