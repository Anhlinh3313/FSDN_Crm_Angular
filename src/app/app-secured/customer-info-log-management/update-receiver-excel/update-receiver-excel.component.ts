...
@Component({
  selector: 'app-update-receiver-excel',
  templateUrl: './update-receiver-excel.component.html',
})
export class UpdateReceiverExcelComponent extends BaseComponent implements OnInit {
  ...
  currentPage: string = Constant.pages.receiver.children.updateReceiver.name;
  msgWarn: string;
  ...
  async setDataNew() {
   ...
   await Promise.all(await this.datas.map((x, i) => {
      if (i >= 1) {
  ...
  async uploadExcelNew() {
    ...
    // tạo
    let rs = await this.customerInfoLogService.updateImportExcelAsync(this.customerInfoLogs);
    this.value = 100;
    ...
    if (rs.isSuccess) {
      this.messageService.add({
        ...
        detail: `Cập nhật thành công ${this.customerInfoLogs.length} KH nhận` ,
      });
      this.clearData();
    }
  }
